...
```text
https://ijudge.it.kmitl.ac.th/submissions/559879/overview
```
...
```text
โจทย์ให้สร้าง Program ในการวิเคราะห์ season โดยจะรับค่าเป็น จำนวนเต็ม 2 ตัว ซึ่งจะเป็นตัวกำหนดวันที่และเดือน

Input:
บค่าเป็น จำนวนเต็ม 2 ตัว ซึ่งจะเป็นตัวกำหนดวันที่และเดือน

Output: 
Program จะ print ออกมาตามเงื่อนไข นั่นคือ season จะมีการเปลี่ยนทุกๆ 3 เดือน แต่มีกรณีพิเศษคือ ตั้งแต่วันที่ 21 ของเดือนที่ 3 หารลงตัวจะต้องขยับไป 1 season

Constraints:
กรณีที่เป็นวันที่ 21 และเดือนที่ 3 หารลงตัว อาจต้องมีการกำหนด  conditions แยกออกมา
```
...
```text
Step 1: รับค่าจำนวนเต็ม 2 ตัว ตัวแรกคือเดือน ตัวที่สองคือวันที่
Step 2: เช็ค conditions
Step 3: ถ้าเป็นวันที่่ต่ำกว่าวันที่ 21 ให้ print ตามเดือน
Step 4: ถ้าเป็นวันที่ 21 ขึ้นไป ให้ขยับไป 1 season
```
...
```text
มีการเปลี่ยนแปลงที่ step 3 ถ้าผมทำแบบนั้นเงื่อนไขมันจะ ไม่ clear เพราะอาจเกิดกรณีที่เป็นเดือนที่ 3 หารไม่ลงตัวแต่เป็นวันที่ 21 ขั้นไป ผมจึงต้องระบุ conditions เพิ่มว่าไม่ว่ายังไงวันที่ 3 หารไม่ลงตัว ต้องเป็น season เดิมตลอด
```
...
```text
เช็คกรณีที่ วันที่ 3 หารไม่ลงตัว แต่เป็นวันที่ 21 ขึ้นไป
```
...
```text
เช็คกรณีที่ วันที่ 3 หารไม่ลงตัว และเป็นวันที่ไม่เกิน 21
```
...
```text
เช็คกรณีที่ วันที่ 3 หารลงตัว  และเป็นวันที่ 21 ขึ้นไป
```
