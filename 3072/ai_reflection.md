...
| Item | Answer |
|---|---|
| OJ problem number/title | OJ3072 - A-E-I-O-U |
| OJ submission ID, if submitted | https://ijudge.it.kmitl.ac.th/submissions/573577/overview |
| OJ status | Pass |
...
```text
1.ช่วยอธิบายแนวทางในการใช้ array กับข้อนี้หน่อยว่าจะใช้ กับ loop ยังไงดี
2.ทำไมต้องใช้ .index 
```
...
```text
1. AI บอกว่าสามารถใช้ loop หาค่าแล้วมาบวกกับ array เดิมที่ตั้งไว้ ซึ่งตั้งเป็น 0 ก่อน 
2. AI บอกว่าเพราะว่าเดิมที่ vowels เดิมเป็นแค่ str ต้องมีการเพิ่มเติมเพื่อให้มันสามารถอ่านได้ว่าต้อง + array เพิ่มที่ตำแหน่งไหน แบบว่า ถ้า ch = 'a' → vowels.index('a')
```
...
```text
เลยลองตรวจสอบที่ index = vowels(ch) เป็น index = vowels.index(ch) เพื่อให้มันสามารถแยก index ออก แล้วเพิ่มค่าได้ถูกต้อง
```
...
```text
1.ได้เรียนรู้ในการใช้ array คู่กับ Loop
2.ได้รู้ว่าก่อนที่เราจะสามารถใช้ให้มันอ่านค่าจาก str ตรงๆแบบนั้นซึ่งมันไม่ใช่ function ต้องมีการกำหนด index ก่อน
```
