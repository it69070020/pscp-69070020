...
```text
https://ijudge.it.kmitl.ac.th/submissions/573577/overview
```
...
```text
โจทย์ให้สร้าง Program ที่จะรับค่ามาเก็บลงใน array แล้วจากนั้นเช็คที่ละตัวว่ามี AEIOU กี่ตัว

Input:
ข้อความ 1 ข้อความโดยที่ไม่สนว่าตัวพิมพ์ใหญ่หรือเล็ก

Output:
จะแสดงจำนวนของสระทุกตัว แบบแยกออกมา

Constraints:
ต้องระวังเรื่องพิมพ์ใหญ่-เล็ก และการเข้า Loop array ให้ดี
```
...
```text
Step 1: รับข้อความ
Step 2: จะเข้า loop วนอ่านทีละตัว
Step 3: จะบวกค่าใน array ของตัวที่ตรง 
Step 4: อีก loop จะทำการอ่านว่าค่าทั้งหมดที่ได้มามีอะไรบ้าง 
Step 5: จากนั้น print เฉพาะตัวที่นับได้ แล้วบอกวา่าตัวนั้นได้กี่ตัว
```
...
```text
วิธีเหมือนเดิมหมดแต่ว่ามีการเปลี่ยนตรง vowels เพราะมันเป็น string ต้องใช้ .index เป็นตัวในการแปลตัวอักษร 
เช่น ถ้า ch = 'a' → vowels.index('a') 
จะเขียนออกมาเป็น all_letter[index] = all_letter[index] + 1
เป็นรูปย่อของ 
if ch == 'a':
    all_letter[0] += 1
elif ch == 'e':
    all_letter[1] += 1
นี่คือส่วนที่เปลี่ยนหลักๆ
```
...
```text
เช็คกรณีที่มีแทรกพยัญชนะ 1 ตัว
```

Input:

```text
aaaaaaaaaaaaaaaaaaaaaammmmmmmo
```
