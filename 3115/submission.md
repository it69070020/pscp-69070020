...
```text
https://ijudge.it.kmitl.ac.th/submissions/630411/overview
```
...
```text
โจทย์ให้ทำ program เช็คจำนวนร้านที่เปิดในช่วงเวลาที่เรากำหนด

Input:
รับค่า  num และ check( จํานวนร้าน และ จํานวนช่วงเวลาที่ต้องตรวจสอบ) บรรทัดแรก
รับค่า num ไปจนครบ ต่อด้วย เวลาที่ต้องการตรวจสอบจํานวน check 

Output:
program จะ print ผลลัพธ์ check ค่า จำนวนร้านที่เปิดอยู่ในเวลานั้นๆ

Constraints:
แต่ละตัวมีการกำหนดขอบเขตไว้เช่น 1 ≤ num ≤ 10⁵, 0 ≤ start < stop ≤ 1440
```
...
```text
Step 1: อ่านค่าจำนวนเต็มที่กำหนด num กับเช็ค 
Step 2: ทำ loop เพื่อรับค่า ทum และ check
Step 3: สร้าง list ไว้เพื่อเก็บค่า 
Step 4: เช็คตามเงื่อนไขว่า start <= k < stop ถ้าตรง count += 1 ( ไล่ loop จาก list stores)
Step 5:  ทำ list result ไว้ โดยจะใส่ค่า count ที่ได้เป็น str
Step 6: print โดยใช้ function join เป็นการเอา str มาต่อกัน
```
...
```text
เหมือนเดิมหมดแต่ตัดการใช้ค่า check ออก เพราะบอกแค่ "จะมีกี่ค่าให้เช็ค" แต่ตอนเราอ่าน check_times = input().split() เราก็ได้ค่าทั้งหมดมาในบรรทัดเดียวอยู่แล้ว ไม่จำเป็นต้องรู้จำนวนล่วงหน้าเพื่อวน loop อ่าน
```
