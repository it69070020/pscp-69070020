...
```text
https://ijudge.it.kmitl.ac.th/submissions/662768/overview
```
...
```text
โจทย์ใหัเขียนโปรแกรมในการคำนวณ แปลงปลูกดอกไม้โดยใช้สูตรอนุกรม

Input:
รับค่าสองตัวคือ L และ N เป็นจำนวนเต็ม (L คือความกว้างแถบ N คือจำนวนช่องที่ปลูกไปแล้ว

Output:
จำนวนเต็มหนึ่งจำนวนระบุหมายเลขของแถบของช่องสุดท้ายที่มีการปลูกดอกไม้ 

Constraints:
ถ้าไม่ถึง N ต้องมีการบวกเพิ่มแล้วคำนวณตาม loop ใหม่
```
...
```text
Step 1: รับค่าเป็น str จากนั้นแยกออกมาเป็น int สองค่า
Step 2: ตั้งให้ m เป็นค่าเริ่มต้นเท่ากับ 1
Step 3: เข้่า while loop โดยใช้สูตร k * (k + 1) // 2 โดย K ได้จาก k = m * L
Step 4: ถ้า total มากกว่าหรือเท่ากับ N ให้ print m และ break loop
```
...
```text
เหมือนกับวิธีแรก แต่เพิ่ม m += 1 เมื่อมันยังไม่ถึง n แล้ววน loop ใหม่ คำนวณ k, total ใหม่ด้วย m ที่มากขึ้น
```
