...
```text
https://ijudge.it.kmitl.ac.th/submissions/648728/overview
```
...
```text
โจทย์ให้ทำ program ในการเช็ค lottery ว่าตรงกับรางวัลมากน้อยแค่ไหน

Input:
รับค่า 2 ตัว ตัวละสองส่วน คืออักษรและเลข โดยจะรับเป็น str ทั้งหมด

Output:
จำนวนเงินรางวัล

Constraints:
ควรรับค่าเป็น str เพื่อใช้ string slicing ในการเช็ค
```
...
```text
Step 1: รับ str ของข้อมูล lottery รางวัลและใบที่ซื้อ
Step 2: นำมาเช็คตามเงื่อนไขที่ละส่วน
Step 3: ใช้ string slicing แยก index แล้วมาเทียบกัน
Step 4: print ค่าเงินที่ได้
```
...
```text
เหมือนกับแผนแรกหมด แค่มีการแก้ไขตรงการกำหนด index เล็กน้อยเพราะช่วงแรกพิมพ์ผิด
```
...
```text
กรณีเป็น 0 ตรงกันหมด (ต้องได้รางวัล)
```
